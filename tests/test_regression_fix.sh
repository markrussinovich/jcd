...
echo "Regression: 'mcd /da' should return immediate matches (/datadrive, /datadrive2)"
...
time ./target/release/mcd "/da" 0 2>/dev/null
echo

echo "Getting all matches for '/da':"
for i in {0..3}; do
    result=$(./target/release/mcd "/da" $i 2>/dev/null)
    if [ $? -eq 0 ]; then
        echo "  Match $i: $result"
    else
